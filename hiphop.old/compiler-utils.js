...
/*    Last change :  Thu Nov 22 11:15:06 2018 (serrano)                */
...
class TrapVisitor {
  ...
  visit(ast_node) {
    if (ast_node instanceof ast.Trap) {
      ...
      this.trapStack = this.trapStack.concat([ast_node.trapName]);
      ast_node.children.forEach(c => c.accept(this));
      this.trapStack = ostk;
    } else if (ast_node instanceof ast.Exit) {
      if (this.trapStack.indexOf(ast_node.trapName) === -1) {
        throw error.SyntaxError(`trap unbound "${ast_node.trapName}".`, ast_node.loc);
      }

      let offset = this.trapStack.length
        - this.trapStack.lastIndexOf(ast_node.trapName) - 1;
