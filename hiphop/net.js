...
/*    Last change :  Wed Mar 11 16:00:10 2020 (serrano)                */
/*    Copyright   :  2018-20 Inria                                     */
/*    -------------------------------------------------------------    */
...
class Net {
   constructor( ast_node, type, debug_name, lvl ) {
      ...
      this.debug_name = ast_node.loc.filename ? `[${debug_id}]:${ast_node.constructor.name}[${path.relative(cwd, ast_node.loc.filename)}:${ast_node.loc.pos}](${this.constructor.name})_${debug_name}:${lvl}/${ast_node.depth}`
      :`[${debug_id}]:${ast_node.constructor.name}(${this.constructor.name})_${debug_name}:${lvl}/${ast_node.depth}`
      ...
      this._dependencyCount = -1;
      this._trueFaninCount = -1;
      this._len = -1;
   }
   ...
   reset( reset_machine ) {
	 ...
	 this._len = len;
	    ...
	    if (fanin.dependency) {
	       ...
	       this._trueFaninCount++;
	    }
      	 }
      }

      this.dependencyCount = this._dependencyCount;
      this.trueFaninCount = this._trueFaninCount;
   }
   ...
   propagate( knownList ) {
      ...
      if( trace ) {
	 console.error( `propagate: ${this.debug_id} value:${this.value}` );
      }
...
class RegisterNet extends Net {
   ...
   receive( value, _ ) {
	...
	//GB too heavy.. console.error( `     receive ${this.debug_id} value:${value}` );
      }
      this.trueFaninCount--;
...
class LogicalNet extends Net {
   ...
   receive( value, fromDep ) {
    ...
    // GB TODO : Old code, should be removed ???
    /*
