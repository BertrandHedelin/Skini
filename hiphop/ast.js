...
/*    Last change :  Tue May  7 14:58:19 2019 (serrano)                */
/*    Copyright   :  2018-20 serrano                                   */
/*    -------------------------------------------------------------    */
...
class Atom extends ActionNode {
  ...
  clone() {
    return new Atom(this.tag, this.id, this.loc, this.nodebug,
...
class Module extends ASTNode {
  ...
  compile() {
    new mach.ReactiveMachine(this).compile();
  }
...
class Run extends ASTNode {
  constructor(tag, id, loc, name, nodebug, sigdecllist, autocomplete, frame, body) {
...
class SignalAccessor {
  ...
  clone() {
    return new SignalAccessor(
      this.signame, this.get_pre, this.get_value, this.is_counter);
  }
}
