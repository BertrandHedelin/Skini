...
var demandeDeSons = "";
var par;
var abletonON = 3; // par défaut 3eme automate
var scoreLoaded;
var refProcessing;
var noScore = false;
...
const Ginactive = 10;
const Binactive = 10;
const colorInactive = '#797bbf' // Violet
...
class Rectangle {
  ...
  display() {
    ...
    this.processing.fill(this.colorRect);
...
class Ellipse {
  ...
  display() {
    ...
    this.processing.fill(this.colorEllipse);
...
class Tank extends Rectangle {
  constructor(indexTank, tankNumber, x, y, color, text, previous, graphicScene, processing){
     ...
     // Pour initialiser le rectangle
      super(x, y, width, height, colorInactive, text, processing); // Attention: "super" à placer avant les références à this
      ...
      this.processing = processing;
      this.active = false;
      this.couleurInactiveTank = colorInactive; //[Rinactive,Ginactive,Binactive];
  ...
  decrement() {
    ...
    if (this.active){
      // Jouer sur l'intensité !!
		...
		super.setColor(colorInactive); //[Rinactive,Ginactive,Binactive]);
	}
...
class Group extends Rectangle {
  constructor(indexGroup, signalCount, x, y, couleur, text, previous, graphicScene, processing){
    ...
    this.graphicScene = graphicScene;
	  this.processing = processing;
	  this.active = false;
	  this.couleurInactiveGroup = colorInactive; // [Rinactive,Ginactive,Binactive];
	  this.name = text;
 ...
 isTank(){
  ...
  activate() {
    if(debug1) console.log("activate:color:", this.colorGroup);
...
function sketchProc(processing) {
  ...
  processing.setup = function() {
    ...
    var scene = 0;
    if (par.groupesDesSons[scoreLoaded][index][8] !== undefined ){
       scene = par.groupesDesSons[scoreLoaded][index][8];
    }
    if (debug) console.log("createTank:", par.groupesDesSons[scoreLoaded][index][0]);
     groups[indexGroup] = new Tank(
      par.groupesDesSons[scoreLoaded][index][1], // index du tank
      par.groupesDesSons[scoreLoaded][index][5], // numéro du tank
      par.groupesDesSons[scoreLoaded][index][3], // x
      par.groupesDesSons[scoreLoaded][index][4], // y
      par.groupesDesSons[scoreLoaded][index][6], // color
      par.groupesDesSons[scoreLoaded][index][0], // name
      par.groupesDesSons[scoreLoaded][index][7], // Antécédents
      scene, // Scène graphique
      processing);
    }

    scoreLoaded = abletonON - 1;
    if (debug) console.log("processing setup:", scoreLoaded );
    groups  = new Array(par.groupesDesSons[scoreLoaded].length);
    
    var groupsCounter = 0;
    var sceneGroupe = 0;

    for (var i=0; i < par.groupesDesSons[scoreLoaded].length; i++) {
	  if (par.groupesDesSons[scoreLoaded][i][2] == undefined) {
	  	alert("ERR: No score defined, choose a score in the controler.");
	  	noScore = true;
	  	return;
	  }else{
		  noScore = false;
	      if (par.groupesDesSons[scoreLoaded][i][2] == "group") {

	        if (debug) console.log("This one is group", par.groupesDesSons[scoreLoaded][i], groupsCounter);
          if (par.groupesDesSons[scoreLoaded][i][8] !== undefined ){
            sceneGroupe = par.groupesDesSons[scoreLoaded][i][8];
          }else{
            sceneGroupe = 0;
          }
	        groups[groupsCounter] = new Group( 
						par.groupesDesSons[scoreLoaded][i][1], // index du  groupe
						par.groupesDesSons[scoreLoaded][i][5], // nbe d'éléments
	        	par.groupesDesSons[scoreLoaded][i][3], // x
            par.groupesDesSons[scoreLoaded][i][4], // y
            par.groupesDesSons[scoreLoaded][i][6], //color
            par.groupesDesSons[scoreLoaded][i][0], //name
            par.groupesDesSons[scoreLoaded][i][7], // Antécédents
            sceneGroupe, // Scène graphique
            processing);
	        groupsCounter++;
	      }else if(par.groupesDesSons[scoreLoaded][i][2] == "tank"){
	        if (debug) console.log("This one is the tank:", par.groupesDesSons[scoreLoaded][i][0]);
	        if ( i > 0){
		        if (groups[groupsCounter-1].isTank()){
			        if (debug) console.log("Previous is the tank:", groups[groupsCounter-1].getName());
			        if(groups[groupsCounter-1].getTankNumber() == par.groupesDesSons[scoreLoaded][i][5]) {
...
*/
  processing.draw = function() {
    ...
    //console.log("draw : display",  groups.length); // !!
      // Quand il ya des tanks, le nombre de groupes sera < au tableau  par.groupesDesSons[scoreLoaded]
	     if (groups[i] == undefined) {
	        //console.log("draw : display: undefined ",  i );
	        break;
	     }
    ...
    // Repérage des antécédents (prev)
	    ...
	    if (IndexPrev.length > 0 ){
	      for (var j=0; j < IndexPrev.length; j++ ) {
	    		...
	    		//if(groups[ IndexPrev[j] ].isActive() &&  groups[i].isActive() ){
          ...
          if(isInOngoingGraphicScene(groups[ IndexPrev[j] ].getGraphicScene()) &&
            isInOngoingGraphicScene(groups[i].getGraphicScene())) {
    			  ...
    			  //processing.line(xStart,yStart,xEnd,yEnd);
    			  processing.noFill();
    ...
    // Message qui prend le dessus du score
...
function isTank(nomDeGroupe){
  var isTank = false;
  for(var i=0; i < par.groupesDesSons[abletonON -1].length; i++){
      if( par.groupesDesSons[abletonON -1][i][0] == nomDeGroupe) {
     		if ( par.groupesDesSons[abletonON -1][i][2] == "tank"){
         	isTank = true ;
         	}
          break;
       }
...
function isTankFromNumOfGroup(numOfGroup){
  var isTank = false;
  for(var i=0; i < par.groupesDesSons[abletonON -1].length; i++){
      if( par.groupesDesSons[abletonON -1][i][1] == numOfGroup) {
     		if ( par.groupesDesSons[abletonON -1][i][2] == "tank"){
         		 isTank = true ;
         	}
          break;
...
function  getTankNumberFromGroupName(nomDeGroupe) {
   ...
   for(var i=0; i < par.groupesDesSons[abletonON -1].length; i++){
    	if( par.groupesDesSons[abletonON -1][i][0] == nomDeGroupe) {
	     	tankNumber =  par.groupesDesSons[abletonON -1][i][5];
	       	if (debug) console.log("Tank number pour groupname:", nomDeGroupe, par.groupesDesSons[abletonON -1][i], tankNumber );
	       	break;
	   	}
	}
	return tankNumber;
...
function  getTankNumberFromNumberInConf(numInConf) {
	var tankNumber;
   for(var i=0; i < par.groupesDesSons[abletonON -1].length; i++){
    	if( par.groupesDesSons[abletonON -1][i][1] == numInConf) {
	     	tankNumber =  par.groupesDesSons[abletonON -1][i][5];
	       	if (debug) console.log("getTankNumberFromNumberInConf:", par.groupesDesSons[abletonON -1][i], tankNumber );
	       	break;
	   	}
	}
	return tankNumber;
...
server.addEventListener('etatDeLaFileAttente', function( event ) {
  ...
  for (var i = 0; i < event.value.length ; i++ ) {
    ...
    console.log("File:", i, "--", event.value[i][2]);
  }
}); 

 /* Info sur changement de partition */
server.addEventListener('abletonStatus', function( event ) {
  if (debug) console.log("Reçu abletonStatus:", event.value );
	if ( event.value > 0 ) {
	  abletonON = event.value ;
	  noScore = false;
